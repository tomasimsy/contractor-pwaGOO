...
">
  {/* ACTIONS STACK */}
  <div className={`flex flex-col items-end gap-2 transition-all duration-200 origin-bottom-right
    ${fabOpen ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-3 scale-95 pointer-events-none"}
  `}>
    {/* EDIT BUTTON */}
    {!isEditMode && !estimate?.signature && estimate?.status !== "converted" && (
      <button
        ...
        className="flex items-center gap-2 rounded-xl text-navy bg-white px-3 py-2 text-sm shadow-md border border-gray-200 hover:bg-gray-50 transition"
        title="Edit Estimate"
      >
  ...
  {/* MAIN FAB - GREEN BUTTON WITH TOGGLE */}
  <button
  ...
  >
    <span className={`text-2xl font-bold transition-transform duration-300 ${fabOpen ? "rotate-45" : "rotate-0"}`}>
      +
    </span>
  </button>
</div>

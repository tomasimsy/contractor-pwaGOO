...
/**
 * Estimate Roof V2 — list page. Reuses estimateService.list() (the
 * exact same `estimates` table V1 reads) — no parallel data source,
 * this is the same estimates, just defaulted to a roofing-only view
 * with links into the V2 routes. Layout mirrors /estimates (desktop
 * table + mobile cards, status/type/sort filters) so the two list
 * pages behave the same way; the only functional difference is this
 * page defaults its Type filter to "Roofing" and its "New" button
 * points at the roofing create route.
 */
...
function EstimateRoofListContent() {
  ...
  // Defaults to "roofing" — preserves this page's original roofing-only
  // behavior; switching it is opt-in, same underlying estimateService.list()
  // as /estimates, not a second data source.
  const [typeFilter, setTypeFilter] = useState<"all" | "standard" | "roofing">("roofing");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load roofing estimates.");
    } finally {
      setLoading(false);
    }
  ...
  }, [estimates, typeFilter, statusFilter, search, sortKey, projectsById, clientsById]);

  // A Standard estimate shown here (typeFilter = "all"/"standard") must
  // route to the V1 detail page, not the V2 roofing route — this list
  // can show both types, but each row's own type decides where it
  // links, never the page's default filter.
  const detailHref = (estimate: Estimate) =>
    estimate.estimateType === "roofing" ? `/estimates-roof/${estimate.id}` : `/estimates/${estimate.id}`;

  return (
    <PageContainer>
      <PageHeader
        title="Estimate Roof"
        description="Roofing estimates — per-area details, measurements, and line items."
        actions={
          <Link href="/estimates-roof/new" className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">
            <Plus className="size-4" /> New Roofing Estimate
          </Link>
        }
      />
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Home}
          title={estimates.length === 0 ? "No roofing estimates yet" : "No estimates match your filters"}
          description={estimates.length === 0 ? "Create your first roofing estimate." : "Try a different search, type, or status filter."}
        />
      ) : (
        <>
          ...
          <div className="space-y-2 sm:hidden">
            {filtered.map((estimate) => (
              <Link key={estimate.id} href={detailHref(estimate)} className="block rounded-xl border border-border bg-card p-3 hover:border-primary/40">
...
export default function EstimateRoofListPage() {
  return (
    <RequirePermission resource="estimate" action="view">
      <EstimateRoofListContent />
    </RequirePermission>
  );
}

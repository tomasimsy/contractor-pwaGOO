...
function invoiceTone(invoice: InvoiceRow) {
  if (invoice.amount_paid <= 0) return { label: "Unpaid", icon: Clock, text: "text-slate-500", bg: "bg-slate-50", border: "border-slate-200" };
  ...
  return { label: "Partial", icon: Clock, text: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200" };
}
              ...
              <div key={invoice.id} className="py-2.5 first:pt-0 last:pb-0 flex items-center gap-3">
                <div
                  className={`shrink-0 w-9 h-9 rounded-full border flex items-center justify-center ${
                    invoice.overdue ? "bg-rose-50 border-rose-200 text-rose-600" : `${tone.bg} ${tone.border} ${tone.text}`
                  }`}
                >
                  <Icon size={15} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <span className="shrink-0 text-xs sm:text-sm font-mono font-bold text-slate-800">
                      {formatCurrency(invoice.amount_paid)} / {formatCurrency(invoice.total)}
                    </span>
                  </div>
                  <div className="text-xs text-slate-400 flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
                    <span className={invoice.overdue ? "text-rose-600 font-semibold" : ""}>
